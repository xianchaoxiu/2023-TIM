...
%
    ...
    while (epoch < 500)  
        ...
        % store info
        [infos, f_val, optgap1] = store_nmf_infos(V, W, H, R, options, infos, epoch, grad_calc_count, elapsed_time);       
        
        % display infos
        if options.verbose > 1
            if ~mod(epoch, disp_freq)
                fprintf('ALS (%s): Epoch = %04d, cost = %.16e, optgap = %.4e, time = %e\n', options.alg, epoch, f_val, optgap1, elapsed_time - prev_time);
            end
        end  
      
        prev_time = elapsed_time;  
        % stopping 
        optgap = max(norm(W-W0,'fro')/norm(W0,'fro'),norm(H0-H,'fro')/norm(H0,'fro'));
        if optgap < 10^-5
            break
        end
