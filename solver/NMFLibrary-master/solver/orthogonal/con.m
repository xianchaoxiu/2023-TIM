function [x, infos] = nmf_hals_so(V, rank, options)
...
%
    ...
    local_options.wo        = 0.1;  % weight of orthogonal constraints for W %需要用交叉验证
...
tic;
fprintf('\n Start to run the solver -- SJOSNMF\n')
fprintf(' -------------------------------------------------------------------------------------------\n')
fprintf(' Iter           wError                  hError                  Error                  Time\n')
fprintf(' -------------------------------------------------------------------------------------------\n')
    % main loop
    while epoch < 500  
        W0 = W;
        ...
        for j = 1:rank
            ...
            W(:,j) = max((W(:,j) + abs(W(:,j)))/2, options.myeps);%replace negative values with zeros (tiny positive velues)
            W(:,j) = W(:,j) / sqrt(W(:,j)'*W(:,j)); %normalize单位化
            wj     = wj + W(:,j);
        end
        ...
        optgap1 = max(norm(W-W0,'fro')/norm(W0,'fro'),norm(H0-H,'fro')/norm(H0,'fro'))
        if optgap1 < 10^-5
            break
        end
    end
    % if options.verbose > 0
      %   if optgap1 < options.tol_optgap
    ...
    %error = norm(Y-W0*H,'fro');
    %H000 = inv(W0'*W0)*W0'*TrainData';
    relerr = norm(max(werror,herror),'fro');
    if relerr <= 10^-5
        
    end
    time = toc;
    fprintf('%d            %8.9f            %8.9f             %8.9f           %4.5f\n',iter,werror,herror,value,time)
end
